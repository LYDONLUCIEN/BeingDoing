...
class ExportService:
    ...
    async def collect_export_data(self, user_id: str, session_id: str) -> Dict:
        """
        收集导出数据
        ...
        async with AsyncSessionLocal() as db:
            ...
            # 获取用户选择
            from app.models.selection import ExplorationResult, UserSelection
            ...
            export_data = {
                ...
                "answers": [
                    {"category": a.category, "content": a.content, "created_at": str(a.created_at)}
                    for a in answers
                ],
                "progress": [
                    {
                        ...
                        "percentage": (
                            int((p.completed_count / p.total_count * 100))
                            if p.total_count > 0
                            else 0
                        ),
                    }
                ...
                "exploration_result": (
                    {
                        "values_selected": (
                            json.loads(exploration_result.values_selected)
                            if exploration_result and exploration_result.values_selected
                            else []
                        ),
                        "strengths_selected": (
                            json.loads(exploration_result.strengths_selected)
                            if exploration_result and exploration_result.strengths_selected
                            else []
                        ),
                        "interests_selected": (
                            json.loads(exploration_result.interests_selected)
                            if exploration_result and exploration_result.interests_selected
                            else []
                        ),
                        "wanted_thing": (
                            exploration_result.wanted_thing if exploration_result else None
                        ),
                        "true_wanted_thing": (
                            exploration_result.true_wanted_thing if exploration_result else None
                        ),
                    }
...
"""
    ...
    def export_to_pdf(self, data: Dict, output_path: str) -> str:
        ...
        try:
            ...
            if result:
                story.append(
                    Paragraph(f"想做的事: {result.get('wanted_thing', '')}", styles["Normal"])
                )
                story.append(
                    Paragraph(
                        f"真正想做的事: {result.get('true_wanted_thing', '')}", styles["Normal"]
                    )
                )
