...
class GuideService:
    ...
    async def should_trigger_active_guide(
        self, session_id: str, last_activity_at: Optional[datetime] = None
    ) -> bool:
        """
        判断是否应该触发主动引导
        ...
        # 计算时间差
        time_diff = datetime.now(timezone.utc) - last_activity_at

        return time_diff.total_seconds() >= timeout

    async def should_trigger_passive_guide(self, answer_content: str, current_step: str) -> bool:
        """
        判断是否应该触发被动引导
    ...
    async def get_guide_preference(self, session_id: str) -> Dict:
        ...
        async with AsyncSessionLocal() as db:
            ...
            if preference:
                ...
                # 默认偏好
                return {"session_id": session_id, "preference": "normal", "created_at": None}

    async def set_guide_preference(self, session_id: str, preference: str) -> Dict:
        """
        设置引导偏好
        ...
        async with AsyncSessionLocal() as db:
            ...
            if guide_pref:
                ...
                guide_pref.updated_at = datetime.now(timezone.utc)
            else:
                guide_pref = GuidePreference(session_id=session_id, preference=preference)
                db.add(guide_pref)
    ...
    async def generate_guide_questions(self, current_step: str, limit: int = 5) -> List[Dict]:
        """
        生成默认引导问题
    ...
    async def generate_active_guide_message(self, session_id: str, current_step: str) -> str:
        """
        生成主动引导消息
        ...
        if guide_questions:
            questions_text = "\n".join(
                [f"{i+1}. {q['content']}" for i, q in enumerate(guide_questions)]
            )
