...
from app.models.session import Progress, Session
...
class HistoryDB:
    ...
    async def create_session(
        ...
        """创建会话"""
        session = Session(
            user_id=user_id, device_id=device_id, current_step=current_step, status=status
        )
    ...
    async def update_session(
        self, session_id: str, current_step: Optional[str] = None, status: Optional[str] = None
    ) -> Optional[Session]:
        """更新会话"""
    ...
    # 进度操作
    async def create_progress(self, session_id: str, step: str, total_count: int = 0) -> Progress:
        """创建进度记录"""
    ...
    async def get_progress(self, session_id: str, step: str) -> Optional[Progress]:
        """获取进度记录"""
        result = await self.session.execute(
            select(Progress).where(Progress.session_id == session_id).where(Progress.step == step)
        )
        return result.scalar_one_or_none()
    ...
    async def update_answer(
        self, answer_id: str, content: Optional[str] = None, metadata: Optional[str] = None
    ) -> Optional[Answer]:
        """更新回答"""
