    ...
    async def append_message(self, session_id: str, category: str, message: Dict) -> Dict:
        ...
        def _do_append(fp: Path) -> Dict:
            ...
            if "event" not in message:
                message["event"] = (
                    "assistant_reply" if message.get("role") == "assistant" else "user_message"
                )
    ...
    async def get_messages(self, session_id: str, category: Optional[str] = None) -> List[Dict]:
        ...
        if category:
            ...
            try:
                async with aiofiles.open(file_path, mode="r", encoding="utf-8") as f:
                    content = await f.read()
                    data = IDCodec.normalize_conversation_data_on_read(
    ...
    async def delete_messages_from_filter_step(
        ...
        def _do(fp: Path) -> int:
            ...
            keys_to_remove = [
                k
                for k in meta
                if k.startswith("step_anchor_")
                and k[len("step_anchor_") :].isdigit()
                and int(k[len("step_anchor_") :]) >= from_step
            ]
