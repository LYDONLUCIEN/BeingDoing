...
class ReportRegistry:
    ...
    def _get_record_via_activation_index(
        ...
        if not record:
            logger.warning(
                "report 索引失效，目标目录不存在，已清空: activation_code=%s report_id=%s",
                code,
                rid,
            )
            self._set_activation_report_index(code, None)
            return None
        if (record.get("activation_code") or "").upper() != code or (
    ...
    def ensure_report(
        self, activation_code: str, user_id: str, session_id: Optional[str] = None
    ) -> dict:
        code = (activation_code or "").strip().upper()
        uid = (user_id or "").strip()
        if not code or not uid:
