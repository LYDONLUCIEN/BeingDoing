...
COMBO_CHAT_SYSTEM_ADDON_ZH = (
    ...
    "禁止在回复中劝用户「手动输入」「复制粘贴」「不用等点击词条」等话术——界面会自动把协议块中的假设渲染为可点击的 chips，你只需保证协议块中每条假设都达到 25 字以上的具体描述。\n\n"
    "输出假设时，必须在协议块之前用一句话引导用户：「你可以点击下方的建议快速填入左侧结论卡」。\n"
    "协议块是在输出假设的同时，在回复正文末尾另起一行输出（界面会自动隐藏）。协议块示例如下：\n"
    "[STEP3_HYP_JSON]\n"
