...
def _cols_step7_final() -> List[Dict[str, Any]]:
    """终步子表：多选 1–3 行（前端用 __pick + rowSelectionMode）。"""
    return [
        {"key": "__pick", "label": "选择"},
        {"key": "id", "label": "id"},
...
GUIDE_TEXT: Dict[int, str] = {
    ...
    3: "请在「假设」列选择：带「个人事业」或「职业路径」色块标签的两条推荐之一，或选「待定」「其他」并自填。至少一行需为有效假设（非「待定」）；全部选「待定」无法进入下一步。确认后进入价值观相关筛选。",
    4: "请为各行选择价值观标签、选「都不符合」「待定」或「其他」并填写后整表确认。",
    5: "请在本页为各行选择「忍不住想做」或「应该做」后整表确认。",
    6: "请在本页为各行选择「现在」或「未来」后整表确认。",
    7: "请勾选 1–3 行最认同的方向（至少 1 行、最多 3 行），点击「确认」后在右侧生成结论卡；在结论卡上最终确认后将进入下一阶段。",
}
