        ...
        base.append(
            {
                "key": "工作目的",
                "label": "工作目的",
                "options": list(values_keywords) + ["都不符合"],
            }
        )
    else:
        base.append(
...
GUIDE_TEXT: Dict[int, str] = {
    1: "请确认您的热爱与优势组合。编辑「优势标记」后点确认；将逐行进行（也可一次确认整表）。",
    2: "这是匹配分析结果。可修改「匹配性」；确认后将为匹配项生成职业假设。",
    3: "请填写「用户确认的假设」（可从三条假设中选或自写）；留空则进入下一轮假设。",
    4: "新一轮假设已生成。请为仍未确认的行填写「用户确认的假设」。",
    5: "最后一轮假设。若仍无法选择，确认后系统将标为「待定」。",
    6: "请为每件事选择能体现的价值（价值观关键词来自前序阶段）；选「都不符合」将筛掉该行。",
    7: "请选择「忍不住想做」或「应该做」；「应该做」将在下一步被筛除。",
    8: "请选择「现在」或「未来」；「未来」将在最后一步被筛除。",
    9: "可合并相似项、修改文字。确认后完成筛选，进入最终选择。",
}
