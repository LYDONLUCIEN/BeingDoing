...
def _coerce_values_list(raw: Any) -> List[str]:
    """将经历-价值观字段统一转为 list[str]。
...
def _keywords_max_for_phase(phase: str) -> int:
    ...
    return max(1, int(v.get("max_keywords", 5)))
...
def build_conclusion_state_injection(
    ...
    # 模型自然出卡不设最小轮数限制（2026-09-02 起）：用户明确点头确认即可出卡
...
def sanitize_pending_conclusion_draft(phase: str, draft: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for k in _EXT_KEYS:
        out.pop(k, None)
    # purpose_progress 仅用于进度跟踪，不写入结论卡
    out.pop("purpose_progress", None)
    out.update(ext)
    return out
...
请**只输出一个 JSON 对象**，不要输出任何其他内容（无 markdown 代码块）：
{
  "keywords": ["使命相关核心词或短语，1~5 个，优先使用第一轮确认的价值观关键词"],
  "summary": "仅自然语言使命总结（2-4段，不含 Markdown 表格/列表）",
  "mission_core": "一句话核心价值概括（用户确认）",
  "mission_detail": "详细解释（可空）",
  "mission_aim": "最终目的一句话（可空）",
  "experience_value_rows": [
    {"experience": "经历一句概括", "values": ["对应价值观1", "对应价值观2"]}
  ]
}
experience_value_rows 最多 5 条，每行 values 为数组（一段经历可匹配多个价值观）；须与用户确认过的经历-价值观一致；无结构化信息时可省略或置 []。
keywords 优先使用第一轮价值观阶段的 5 个关键词子集；如需新概括词（最多 2 个），须在 summary 中解释其与已有价值观的关系。
"""
        )
