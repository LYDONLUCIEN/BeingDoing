...
CONCLUSION_PROMPT = """你是「寻路·LifeAsk」系统的结论卡生成器。下面是用户与引导师关于一个组合的对话摘要。请基于摘要，生成一张结论卡。
...
SUMMARIZER_PROMPT = """你是「寻路·LifeAsk」系统的对话摘要器。请把"上一份摘要 + 最近对话"压缩成一份新的摘要，用于引导师继续推进。
