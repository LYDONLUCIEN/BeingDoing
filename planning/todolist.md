...
- [ ] 实现问题引导工具（QuestionGuideTool）
- [ ] 实现示例推荐工具（ExampleRecommendTool）
- [ ] 实现内容检索工具（ContentSearchTool）
...
**任务**：
- [ ] 实现生成结果接口（POST /results/generate）
- [ ] 实现导出接口（GET /results/{id}/export）
- [ ] 实现导出状态接口（GET /results/{id}/export-status）
- [ ] 实现异步导出（可选）
