...
## 八、ADR-0014 交付模型变更（2026-08-05，已实施）

1. **年度套餐调价**：¥128 → **¥159**（三人价，3 个码；季度 ¥69 不变）。
2. **全量未绑定码交付**：套餐订单不再自动升级试用码、不再自动绑定自用码——季度发 1 个、年度发 3 个未绑定完整码，全部可自用/转赠（取消自用码/赠品码区分）。
3. **消耗升级**：`POST /simple-auth/codes/apply-to-trial` 作废 1 个未绑定码（`status=consumed` + `consumed_into` 审计），试用码走 `upgrade_to_full` 原地升级。入口：支付结果页弹窗（「不再提醒」存 `users.preferences`）、两种 402 拦截点（直购 intent=upgrade_trial 支付后自动消耗升级 / 用已有码升级）、我的激活码页。
4. **退款口径**：套餐订单任一码被 claim/consumed → 整单不可退；全部未动 → 可退并 revoke 全部码（旧行为是套餐一律拒退，因旧模型交付即绑定）。
