...
补充：`STEP_OPENING_MODE` 决定该步是 `fixed` 还是 `llm`。默认仅 **第 4 步**为 `llm`，走 `build_step_6_opening_llm_messages`（注意：函数名带 `6` 是历史命名，实际在 `filter_step == 4` 时调用）。
...
| `domain/rumination_step_guidance.py` | `STEP_OPENING_FIXED_ZH`, `STEP_OPENING_MODE`, `STEP_6_OPENING_*` | 沉淀子步引导；第 4 步是否 LLM |
...
| `rumination_step_guidance.build_step_6_opening_llm_messages` | 仅在 **step==4** 调用，函数名遗留 |
