# ADR-0012: 品牌更名——寻路 (xunlu) → 寻路·LifeAsk (lifeask)
...
产品原品牌为「寻路 (xunlu)」，生产域名 `xunlu.beyondego.me`。因品牌升级需要，全面更名为「寻路·LifeAsk」：

- **中文语境**：`寻路·LifeAsk`（间隔号 U+00B7 连接，不连写、不用全角・）
- **英文/域名/代码语境**：`LifeAsk` / `lifeask`（域名、文件名、配置值一律小写）
- **新生产域名**：`lifeask.beyondego.me`
...
1. **品牌书写口径**：中文一律「寻路·LifeAsk」，英文一律 LifeAsk/lifeask；旧称「寻路 / xunlu / Xunlu / XUNLU」在新文案中禁用。术语已沉淀至 `CONTEXT.md`「品牌名 (Brand Name)」。
2. **域名迁移**：`xunlu.beyondego.me` → `lifeask.beyondego.me`，旧域名 nginx 301 永久跳转到新域名；CORS 白名单只保留新 origin。
3. **支付宝回调**：`ALIPAY_NOTIFY_URL` / `ALIPAY_RETURN_URL` 从 `career.beyondego.me` 迁至 `lifeask.beyondego.me`（需在支付宝开放平台同步修改授权回调域）。**这是本次唯一触碰的 career 项**——借此统一生产域名口径。
4. **品牌图片**：PDF 报告用 4 张图（主 logo / 页眉 / 页脚 / 水印）由产品侧重制，图内文字「寻路·LifeAsk」，文件重命名为 `lifeasklogo.png` / `lifeasklogo_header.png` / `lifeasklogo_footer.png` / `watermark_logo.png`，代码同步改引用。
5. **不改的部分**：
