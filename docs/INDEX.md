# 文档索引

## 📚 文档导航

### 🚀 快速开始
- **[快速开始指南](QUICK_START.md)** - 5分钟快速启动
- **[环境配置说明](ENV_SETUP.md)** - 详细环境配置
- **[依赖清单](DEPENDENCIES.md)** - 所有依赖说明

### 🛠️ 开发文档
- **[开发指南](DEVELOPMENT.md)** - 开发环境、代码规范、开发流程
- **[测试说明](TESTING.md)** - 测试运行、编写、调试
- **[API文档](API_DOCUMENTATION.md)** - 完整API接口文档
- **[架构设计](ARCHITECTURE.md)** - 系统架构和技术栈
- **[数据库设计](DATABASE_SCHEMA.md)** - 数据库表结构
- **[Simple 模式数据存储](DATA_STORAGE_SIMPLE.md)** - data/simple 目录、Thread 概念与同步逻辑

### 🚀 部署文档
- **[部署指南](DEPLOYMENT.md)** - 本地和服务器部署
- **[Docker使用](DOCKER.md)** - Docker部署说明

### 👤 用户文档
- **[用户指南](USER_GUIDE.md)** - 用户使用说明
- **[常见问题](FAQ.md)** - FAQ

### 📊 项目文档
- **[项目状态](PROJECT_STATUS.md)** - 开发进度和状态
- **[Mermaid渲染说明](MERMAID_RENDERING.md)** - 流程图渲染

### 📝 其他文档
- **[手动设置](MANUAL_SETUP.md)** - 手动环境设置
- **[快速设置](QUICK_SETUP.md)** - 快速手动设置

## 🎯 按角色查找

### 我是开发者
1. [快速开始指南](QUICK_START.md)
2. [开发指南](DEVELOPMENT.md)
3. [测试说明](TESTING.md)
4. [API文档](API_DOCUMENTATION.md)

### 我是运维人员
1. [部署指南](DEPLOYMENT.md)
2. [Docker使用](DOCKER.md)
3. [环境配置说明](ENV_SETUP.md)

### 我是用户
1. [用户指南](USER_GUIDE.md)
2. [常见问题](FAQ.md)

### 我是架构师
1. [架构设计](ARCHITECTURE.md)
2. [数据库设计](DATABASE_SCHEMA.md)
3. [项目状态](PROJECT_STATUS.md)

## 📋 文档更新记录

- 2024-01-01: 初始文档创建
- 文档位置: `docs/` 目录
- 设计文档: `planning/` 目录
