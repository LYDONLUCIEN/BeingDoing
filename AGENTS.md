...
- **Chat 页外观配置系统（2026-09-21 起，复刻 `wiki/开发文档/0919/openlife-journey (21).html` #appearance-popover 全维度）**：chat 页（前四阶段 header + rumination v4 header）齿轮按钮弹「页面设置」面板，14 个维度——聊天背景（white/tint/flow/illustration）、贴图位置（both/sidebar/edge/off，取代旧 sidebarArt 开关）、晕染浓度（0-50 → `--chat-wash-opacity`）、动效开关、AI/我的气泡（ink/soft/theme/white）、发送与主按钮（ink/theme）、沉淀布局（classic/studio/guided，v4 CSS 排布变体）、沉淀皮肤（folio/modules/editorial/mist，前置：`RuminationV4Page` 外壳/内层内联样式已类化为 `.v4-outer-shell`/`.v4-inner-pane`）、沉淀氛围（lavender/sage/slate）、选择矩阵样式（soft/outline/solid）与配色（duo/violet/multi）、结论卡配色（theme-mist 等 5 态）、结论标签（soft/outline/editorial）+「恢复推荐组合」。链路：`stores/chatAppearanceStore.ts`（zustand persist `openlife-chat-appearance` v1，v0→v1 migrate 把 sidebarArt 折算 placement）→ `lib/explore/useChatAppearanceAttrs.ts` 输出根节点 `data-*` + 内联 CSS 变量 → `styles/components/openlife-chat-appearance.css` 的 `.flow-light[data-xxx]` 覆盖规则。密度/新建对话按钮两个 A/B 开关保留在「对话排版」组。纯前端 localStorage，不接后端 preferences。
