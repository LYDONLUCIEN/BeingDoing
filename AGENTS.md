...
- **结论卡最小出卡轮数门控（2026-08-25 起）**：四阶段（values/strengths/interests/purpose）所有用户**前 10 轮不出结论卡**——用户消息数（含当前条）< `CONCLUSION_MIN_USER_TURNS`(11) 且 `state=none` 时，模型输出 `pending_ready` 也被拦截（记日志 `[state_json] pending_ready suppressed by min-turns gate`），注入段经 `build_conclusion_state_injection(gate_active=True)` 改为「深入探索期不收口、不输出 STATE_JSON」。仅拦截首次出卡：rejected（出过卡被否定）线程的重新出卡/retrigger 不受限；admin 调试（`_can_bypass_flow_limits`）不受限。配套手动兜底：满 11 轮且**当前无待表态卡**（从未出过，或「再聊聊」后旧卡折叠——rejected 状态下按钮恢复显示，防止 retrigger/模型重新出卡双双失效后用户无入口）时，前端输入框上方显示「对话结束无法进行下一步?点击这里」按钮（首次冒泡长出动画 + 闲置 10s/60s 抖动提醒各一次封顶 2 次；**点击后冻结输入框与发送、复用「正在生成结论卡片」提示条，防止生成期间新请求进入**）→ `POST /simple-chat/conclusion/request` 直接走 `check_dimension_complete(skip_completion_check=True)` 生成草案置 pending（与 retrigger 同链路；已有 pending/confirmed 幂等返回；<11 轮 400；生成失败 503 前端「生成失败，点击重试」不静默）。关键文件：`simple_chat_routes.py`（门控 + `request_conclusion_card`）、`app/domain/conclusion_card_payload.py`（gate_active 注入）、前端 `components/explore/ConclusionRequestButton.tsx` + chat 页接入；测试 `test/backend/test_conclusion_min_turns_gate.py`。
