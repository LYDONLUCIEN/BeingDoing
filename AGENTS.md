...
# LLM 余额监控（2026-09-06 起，当前仅支持 DeepSeek）：定时查 GET /user/balance，
# 低于阈值时给所有 super_admin 发站内信（type=llm_balance_low，当天幂等）+ admin 页顶部琥珀色告警横幅；
# key/base_url 走 resolver.get_default_config()（DB 优先 + env 兜底）；job 注册在 main.py _start_bounce_scheduler，
# admin 端点 GET /admin/llm-balance 实时查询；关键文件 app/services/llm_balance_monitor.py
LLM_BALANCE_ALERT_THRESHOLD=10.0        # 告警阈值（元）
LLM_BALANCE_SCAN_INTERVAL_MINUTES=60    # 扫描间隔（分钟）

# LLM per-turn 诊断日志（2026-09-21 起）：主聊天流每轮落一条 jsonl（含 CoT/正文全文，敏感），
# 存 data/logs/llm_turns/llm_turns-YYYY-MM-DD.jsonl；admin「轮次诊断」tab 查询（仅 super_admin）；
# 关键文件 app/services/llm_turn_log_service.py
LLM_TURN_LOG_RETENTION_DAYS=30        # 保留天数，每日 job 清理超期文件
LLM_TURN_LOG_CLEANUP_CRON="0 5 * * *"   # 默认每日 05:00 清理（含空格的 cron 值必须加引号，否则 start.sh source .env 报错）

# 可选：SMTP 邮件（忘记密码功能）
# 统一注脚（2026-09-21 起）：EmailService.send_email 对所有外发邮件统一追加「本邮箱为系统
# 发送邮箱，不接收回复，咨询请联系 openlife.lab@outlook.com」（纯文本+HTML 都加，
# 常量在 app/services/email_service.py 顶部 NOREPLY_NOTICE_TEXT/HTML），新邮件自动带上
...
# 折扣券有效期（2026-09-14 起，迁移 020）：券表加 expires_at/owner_user_id/voided_at，
# 存量券追溯为迁移时刻+90 天。默认有效期走 admin 运行时配置（支付管理页，存
# data/coupon_config.json，GET/POST /admin/coupon-config，即时生效只影响新券，
# 回退默认 90 天，合法域 1-3650），admin 创建时可按批 ttl_days 覆盖；无 env 变量。
# 状态机 unused/locked/used/void（expired 为 unused 的惰性派生态不落库、无定时 job）；
# 过期判定只在「unused 被使用」瞬间（validate/lock），locked 免疫；作废=软删除可恢复
# （POST /admin/coupons/{id}/restore），过期券 PATCH expires_at 改期即复活，used 不可恢复。
# 归属：邮件发券发放即绑定收件人，未绑定流通券下单锁定时认领（校验不认领防扫号），
# 已绑定券他人不可用；用户侧 GET /payment/my-coupons + /dashboard/codes 第三个 tab。
# 退款退券：admin_refund 成功即把本单券 used→unused 退回（保留归属；修复历史退款不退券漏洞）。
# 单券限制（一单一 coupon_code）与超额截 0（max(0, base-coupon)）为既有硬口径，勿破坏。
...
- **「完成并继续」二次确认弹层（2026-09-14 起）**：前四阶段（values/strengths/interests/purpose）chat 页顶栏「完成并继续」点击后先弹 `ContinueConfirmModal`（告知进入下一阶段后本阶段对话与结论卡锁定不可修改、不确定可点左侧「新建对话」继续挖掘），确认才走 `handleCompleteAndContinue` 导航；已提交锁定（stepLocked）阶段本已不可逆，跳过弹层直接导航。沉淀阶段（rumination）顶栏按钮不受影响（走自身终选弹窗流程）。关键文件：`components/explore/ContinueConfirmModal.tsx`、chat 页 `handleRequestCompleteAndContinue`、i18n key `explore.chat.continueConfirm*`。
...
- **空回复自动重试与 LLM 轮次诊断日志（2026-09-21 起）**：背景是「flash 只吐思维链、content 为空被静默吞掉」事故（复盘见 `wiki/开发文档/0921/开发问题.md`）。①`_is_reasoning_model()` 判定放宽为 `reasoner` 或 `deepseek-v4` 前缀（V4 全系默认 thinking=on，flash 也走思维链分流，前端单行预览、不可展开全文）；②主聊天流（`simple_chat_routes.py` `event_stream`）可见正文（`stream_hidden_filter` 后）为空即当失败：推 `retrying` SSE 事件告知并**原样重试 1 次**，仍空推 `error`（值为 JSON 字符串 `{"type":"empty_response"}`，不落助手消息、不推 done），前端错误条+「重新尝试」（i18n `explore.chat.retrying`/`emptyResponseError`）；③**重试去重**：append 用户消息前检测历史最后一条为相同 user 消息且距今 <5 分钟则跳过（前端「重新尝试」会原样重发，避免对话 JSON 出现重复 user 消息）；④**per-turn 诊断日志**：`app/services/llm_turn_log_service.py` 按天写 `data/logs/llm_turns/llm_turns-YYYY-MM-DD.jsonl`（含 CoT/正文全文、outcome 分类 ok/length/empty_content_retried_ok/empty_content_failed/partial/error/disconnected、finish_reason、usage、attempts），admin「埋点与 Token 统计 → 轮次诊断」tab 可查（`GET /admin/analytics/llm-turns[/{id}]`，仅 super_admin），每日 job（`llm_turn_log_cleanup`）按保留期清理。测试：`test/backend/test_chat_empty_reply_retry.py`、`test_llm_turn_log_service.py`。
- **沉淀（rumination）v3 前端彻底删除（2026-09-22 起）**：rumination 无条件走 v4——chat 页 `app/(main)/explore/chat/[phase]/page.tsx` 外层 `ChatPhasePage` 对 `phase === 'rumination'` 早退到独立入口组件 `components/explore/RuminationV4Entry.tsx`（自含 activationCode 获取 +「完成并继续」导航 + LegacyBrowserNotice，dynamic import `ruminationV4/RuminationV4Page`），`LiveChatPhasePage` 变为纯四阶段组件；v3 的 AB 版本判定（`fetchRuminationVersion` / `?v4=1` / `?v3=1`）与全部 v3 代码一并删除：`RuminationTableWidget.tsx`、`RuminationSectionProgress.tsx`、`components/explore/step3/`、`lib/api/rumination.ts`、`lib/explore/{useComboGuideStream,ruminationStepOpening,ruminationStepOpeningFlags,ruminationProgressNav,ruminationStepBoundaries}.ts`；`lib/explore/threads.ts` 删 rumination-only 导出与 ThreadMessage v3 字段，`sessionRecovery.ts` 去 v3 分支，i18n 删 `explore.chat.ruminationUi/ruminationTable/ruminationProgress` 等 v3 key（zh/en 对齐），`styles/components/rumination-beautiful.css` 仅保留 v4 依赖段。后端 v3 接口/判定保持不动。
- **Chat 页外观配置系统（2026-09-21 起，复刻 `wiki/开发文档/0919/openlife-journey (21).html` #appearance-popover 全维度）**：chat 页（前四阶段 header + rumination v4 header）齿轮按钮弹「页面设置」面板，14 个维度——聊天背景（white/tint/flow/illustration）、贴图位置（both/sidebar/edge/off，取代旧 sidebarArt 开关）、晕染浓度（0-50 → `--chat-wash-opacity`）、动效开关、AI/我的气泡（ink/soft/theme/white）、发送与主按钮（ink/theme）、沉淀布局（classic/studio/guided——classic/studio 为 v4 CSS 排布变体；**guided 为结构性三栏**（2026-09-22 起）：`RuminationV4Page` 按 `ruminationLayout==='guided' && !stacked` 改渲染为 左`V4ComboSidebar`（新建组合+纵向列表，复用 store）/ 中对话（未确认组合时盖柔雾锁屏卡，同时给 `V4ChatPanel` 传 `hideDraftHint` 隐藏其内置草稿提示防双遮罩）/ 右选择器，TopComboBar 隐藏，窄屏 stacked 回退 classic；**guided 选择器自动收拢**（2026-09-22 起）：确认组合（开始探索、`active_combo_id` 落定）后右选择器自动收拢、对话区与输入框自动伸展，右缘留 22px «/» 边条（lucide ChevronsLeft/Right）手动切换，回到草稿态（新建组合）强制展开；展开宽度上限 `max(280px, min(430px, calc(50% - 244px)))`——选择器+左组合列表合计不超过半页，另一半留给对话区。**选择器改纯文字小卡**（2026-09-22 起，`V4ComboMatrixSelector`）：删除 ♥/♡ 字符大图标圆圈（`DimensionOption.glyph` 废弃留兼容），标题改 13px 小序号圆+柔色文字，duo 默认配色改热爱珊瑚 #e86f7e × 优势雾蓝 #5b84e6（`--matrix-*` 变量驱动，violet/multi/outline/solid 覆盖规则同步去 `.round-icon`）、选中态浅底色+彩色描边）、沉淀皮肤（folio/modules/editorial/mist，前置：`RuminationV4Page` 外壳/内层内联样式已类化为 `.v4-outer-shell`/`.v4-inner-pane`）、沉淀氛围（lavender/sage/slate）、选择矩阵样式（soft/outline/solid）与配色（duo/violet/multi）、结论卡配色（theme-mist 等 5 态）、结论标签（soft/outline/editorial）+「恢复推荐组合」。链路：`stores/chatAppearanceStore.ts`（zustand persist `openlife-chat-appearance` v1，v0→v1 migrate 把 sidebarArt 折算 placement）→ `lib/explore/useChatAppearanceAttrs.ts` 输出根节点 `data-*` + 内联 CSS 变量 → `styles/components/openlife-chat-appearance.css` 的 `.flow-light[data-xxx]` 覆盖规则。密度/新建对话按钮两个 A/B 开关保留在「对话排版」组。纯前端 localStorage，不接后端 preferences。
- **消息工具栏常显（2026-09-21 起）**：AI/用户气泡工具栏（复制/点赞/检查点）原 `opacity:0` 仅 hover 显示（触屏不可见），改为常显——`flow-chat-light.css` 删两条 hover 规则，`V4ChatPanel` 用户气泡工具栏删 Tailwind `opacity-0 hover:opacity-100`。无点踩/重新生成（v1.6.x 即无）。
...
- **购买入口改版（2026-09-14 起）**：首页定价区块（免费/季度/年度/咨询 4 卡 + `PricingCompareTable` 对比表，`home.pricing.*` i18n 已删）整块下线，首页不再展示价格；购买入口改为**顶部导航栏「定价方案」按钮**（TopNavbar 桌面+移动端，未登录先弹登录），全站弹 PurchaseModal。**2026-09-21 起导航「定价方案」按钮下线**（TopNavbar 桌面+移动端按钮与 handlePricingClick 已删，PurchaseModal 逻辑保留供后续入口复用）；同时导航项调整为：首页 / 开始探索 / 职业成长（新增 `/career` 占位页，与 `/community` 同款「正在开发中」）/ 交流社区（原「社区」改名，`nav.community` i18n 改文案、路径不变）/ 关于我们，配置在 `lib/nav.ts`。
- **首页底部小红书二维码（2026-09-21 起）**：共享组件 `components/common/XiaohongshuQrEntry.tsx`（小图标按钮 lucide QrCode → 点击弹窗显示完整二维码卡片，`public/assets/openlife-journey/xiaohongshu-qr.webp?v=20260921`，PNG 母版同目录保留），两个挂载点：LandingFooter 链接区「关于我们」左侧（`app/(main)/page.tsx`，`.ol-footer-qr`）、/about 文字面板右上角（`.ol-about-qr`）。弹窗纯 CSS（`.ol-qr-modal-*`，openlife-reference-home.css），点遮罩/关闭按钮/Esc 收起；**弹窗经 createPortal 挂 body**——footer z-index:5 的 stacking context 会压住弹层导致盖不过 TopNavbar（z-50），勿改回组件内联渲染。
- **站内信悬浮球可拖拽（2026-09-21 起）**：`FloatingFeedbackWidget` 右下 pill（`.ol-msg-launcher`）支持 pointer 拖拽换位——6px 阈值区分点击/拖拽，pointer capture + 拖拽后抑制补发 click 防误开抽屉；**边缘半吸附**：水平方向最多露出半边（clamp 域 `[-w/2, innerWidth-w/2]`），松手时越界即吸附（右越界只露左半边贴右缘 / 左越界只露右半边贴左缘，`snapToEdge`），未越界保持原位，垂直方向不出屏；位置存 localStorage（`ol-msg-launcher-pos`，left/top 视口坐标，吸附态 x 可为负值），未拖过走 CSS 默认定位（chat 页 `--chat` 上浮避让仍生效），窗口 resize 自动钳回可视区；CSS 需保持 `touch-action: none`（移动端拖拽前提）与 `white-space: nowrap; overflow: hidden`（半隐时防文字换行变形）。PurchaseModal 套餐下单视图改版为宽弹窗两列布局（UI 参考 `wiki/开发文档/0914-openlife-purchase.html`：左栏标题+按套餐切换的权益卡、右栏套餐单选卡+渠道+券码、底部「各方案功能对比」折叠表+固定结算 footer），延期/咨询/等待支付/成功视图保持窄弹窗不变；下单/券码/支付宝收银台/轮询/交付/消耗升级逻辑全部不变（新文案在 `payment.dialog.*`/`payment.compare.*`）。
...
- `/api/v1/search/*` - 检索
- `/api/v1/formula/*` - 公式
- `/api/v1/export/*` - 导出（`POST /export/report-pdf/{id}` 有完成度门控：五阶段未完成一律 409，admin 也不例外，2026-08-07 起；admin 豁免仅限审核状态阻塞。审核计时锚点（2026-08-23 起前移）：rumination v4 终选提交（`final-selection/submit`）成功即 `start_review`（**固定 24h**，2026-09-06 起，原随机 3~24h，存量 pending 不溯及既往）+ 后台预生成 markdown；审核期内自动批复扫描 job（每 10 分钟）兼任**生成看门狗**——pending 未过期且 markdown 缓存缺失/无人生成时自动重试预生成（距上次 ≥1h、上限 3 次，计数落 record.json `pregen_retry_count`/`pregen_last_retry_at`），保证 24h 解锁时报告已就绪，进报告页（`my-report-id`）的 `_ensure_review_started` 保留作存量 not_started 懒触发兜底；报告批复通过瞬间——人工 `POST /admin/reports/{id}/approve` 或超时自动批复 job——后台兜底再生成报告 markdown（`report_review_service.kick_report_generation`，2026-08-10 起）。用户侧「生成报告」按钮已于 2026-08-23 下线：approved 且无缓存/生成失败时改为警示块（失败→「重新尝试生成」+「申请复核」，无缓存→「报告异常」+「申请复核」），下载按钮带「下载中…」态（`useReportPdfDownload.downloading`）。**报告复核体系（2026-08-18 起，`tasks/report-review-plan.md`）**：用户侧重新生成已下线（按钮 + `_MAX_USER_REGEN` + `report_regen_count` 计数全删），改「申请复核」——`POST /export/report-recheck/{id}` 分类必选（`content_issue` 建复核单 + 双写 Feedback 工单 / `download_issue` 仅进 Feedback 通道同「反馈 bug」；**限流 2026-09-14 起改为每报告终身 1 次免费复核**——提交即消耗（驳回也算）、存量复核单溯及既往（`has_used_recheck`），原「每天限 1 次」已废；download_issue 不受限；有未关闭单仍 409）；**复核选项文案（2026-09-14 起）**：content_issue=「内容与你的情况不符、表述不准确，由管理员在5个工作日内复核」、download_issue=「3个工作日内回复处理结果」，弹窗选中 content_issue 时顶部常驻琥珀警示条「每份报告仅有 1 次免费复核机会」；`GET /export/my-report-id` 新增 `recheck_used` 字段，用完后报告页按钮置灰「复核机会已用完」（tooltip 引导走「反馈 bug」/邮件）；**套餐卖点（2026-09-14 起）**：季度/年度套餐 features 各加「1 次免费报告复核」（payment_service.get_products + i18n `payment.plan.*.f4/f5`，PurchaseModal planFeatures 遍历 f1..f5）；复核单存 record.json `recheck_requests`（append-only，状态机 pending→regenerating→pending_confirm→done/rejected，LLM 失败退回 pending 可重试）；重新生成写 staging `report_markdown.staging.md`（PDF 即时渲染故用户始终看旧版，报告不锁定只加提示条），admin 确认发布才原子替换正式缓存（旧版留 `.bak`）+ 站内信 + 邮件；admin 端点 `/admin/reports/{id}/recheck/*`（regenerate / staging-pdf / publish / reject 必填理由 / **markdown（original|staging 原文，2026-09-14）/ publish-edited（admin 编辑稿直发正式缓存，2026-09-14）**），「重新生成」按钮仅随未关闭复核单出现、期间可反复生成、关闭即消失，后端 force 保留供运维脚本；**Admin Markdown 编辑器（2026-09-14，`components/admin/RecheckMarkdownEditor.tsx` 嵌于 ReportRecheckPanel）**：原版与新稿双版本 tab、按 H2 分章查看/编辑（textarea + `@uiw/react-markdown-preview` 实时渲染，非 PDF）、本地下载 .md、选其一「发布此版本」（编辑仅存前端、发布时整体提交，未选中的由后端清除 staging），发布走 `report_recheck_service.publish_edited_recheck` → `report_pdf_service.publish_edited_markdown`（旧版 .bak + 原子写入 + 时间戳刷新 + 关单通知）；关键文件 `app/services/report_recheck_service.py`、`report_pdf_service` staging 方法、前端 `components/explore/ReportRecheckModal.tsx` + `components/admin/ReportRecheckPanel.tsx`）；三条生成路径（审核预生成/批复/手动）共用 `report_pdf_service` 单轨锁，同报告不并发。**报告 prompt 终选硬约束（2026-09-06 起）**：`report_system.yaml` 第五章「最终选择」的分析范围严格以注入的「沉淀阶段最终结论」（rumination N选3 终选结论卡，数据区标注为权威数据源）为准，禁止模型从对话全文自行摘取/更换/新增方向；`_collect_rumination_block` 同步注入每个选中方向的**平衡点判定**（`balance_analysis`/`conclusion_card` 双字段兼容：已找到平衡点（推荐）/未找到平衡点+卡点；**None（未判定/证据不足/v3 存量）不注入该行**（2026-09-08 修订，防止模型把「未作判定」元信息写进正文）），prompt 要求第五章如实体现判定结果。**沉淀阶段对话过滤（2026-09-09 起）**：`conversation_block` 中沉淀阶段不再注入 session 全文，改由 `_collect_rumination_selected_dialogue` 只注入 N选3 选中方向的对话（v4 `combo_sessions[].messages` 按组合分存精确过滤），未选中候选方向的对话不进 prompt（选中方向无对话时降级为说明文案），prompt 数据区同步加注「报告任何章节不得出现未入选方向」。PDF 页面顺序（2026-08-16 起）：封面 → 阅读指南 → 报告内容总览（原「报告模块总览」，已去掉日期/版本/密级档案头）→ 其余章节（`_markdown_to_pdf` 在第一个分页符处切开正文插入总览页，找不到分页符降级为旧顺序）；正文表格有行级分页保护（`.content tr/th/td { break-inside: avoid }`，防同一行左右格被拦腰分到两页）；「开篇：职业角色」章由后处理规范化（ADR-0017：`_normalize_role_opener` 保证 H2 标题必有，渲染侧 `_markdown_to_pdf` 也跑、存量缓存下载即生效；描述 >800 字时 `_compress_role` LLM 压缩至 600-750 字，仅生成侧，验收失败回退原文））。**渲染引擎双轨（ADR-0019，2026-08-20 起）**：`RENDER_ENGINE=weasyprint`（默认）| `xunlu`（`src/report-renderer/` 精简 Node 渲染器，抽自只读母版 `report/xunlu`，子进程调用，契约 markdown→bytes 不变），分流点 `_markdown_to_pdf`，用户下载与 admin staging 预览同时生效；xunlu 引擎自带存量方言归一化（旧式分页符/`*` 列表/h5-h6/`##`·`###` 章节标题提升/动态角色名识别）；xunlu 分页为 JS 估算分页（`markdown-parser.ts` paginateReport，2026-08-31 已按 report.css 校准行高/每行字数常数：表格 15.5px/行、每格字数按 694px 实宽÷列数、段落 53 字/行），跨页拆块（段落/列表/表格）打 splitGroup 同源标记，rebalanceSectionEnd 合并尾页时按标记拼回——修复同页出现「表格中间断开+重复表头/段落拦腰截断」）
...
8. **Refresh 轮换宽限期（2026-08-25 起）**: `REFRESH_TOKEN_ROTATE_GRACE_SECONDS`（默认 10s）——已轮换旧 refresh token 在宽限期内被重放视为**并发刷新**（多标签页/流式与轮询同时 401），照常换发不撤族；宽限窗外维持 `reuse_detected` 撤销整族的防盗语义；窗口锚定首次轮换时间（重放不续期，rotation 分支仅在 `revoked_at is None` 时写）。前端所有 refresh 统一收敛到 `apiClient.refreshAccessToken()`（single-flight，`client.ts`），聊天页流式/ruminationV4Api 的 401 处理均走该入口，不再自行调 `authApi.refresh()`。另修复 SQLite 下 `refresh_access_token` 对 `rec.expires_at`（naive datetime）与 aware now 比较抛 TypeError 致 refresh 500 的问题（按 UTC 转 aware 再比较）。测试 `test/backend/test_refresh_token_rotation.py`
