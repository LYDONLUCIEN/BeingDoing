...
`filter_step=3` 时，用户前面探索出的热爱（最多3个）与优势（最多5个）构成 3×5=15 个自由组合。每个组合有独立的对话记录和假设结论，combo_id 格式为 `passion_idx + strength_idx`（如 `"02"` = 热爱1×优势3）。

**组合假设状态 (Combo Hypothesis State)**
每个组合有三种状态：`empty`（未填写）、`confirmed`（已确认假设）、`skipped`（已跳过，保留文字但灰显标记）。跳过分两种：纯空跳过和有字跳过（后者值得在全部提交时追问）。
